...
class KodTranslator:
    ...
    @staticmethod
    def boolconst_to_kod(d: RelationDecl, index: int) -> KodExpr:
        if not d.mutable:
            index = 0
        return f'this.get_relation("{d.name}", {index}).some()'
    ...
    def _translate_expr(self, e: Expr) -> KodExpr:
        ...
        # TODO: CHANGE TO ELIFs and don't leave it open-ended
            ...
            if isinstance(d, RelationDecl): # Nullary relation : BOOL
                return KodTranslator.boolconst_to_kod(d, self.scope.current_state_index)
...
class KodSolver:
    ...
    def kod_get_constructor(self) -> List[str]:
        ...
        lines.extend([
            ...
            'this.relations = new HashMap<Map.Entry<String, Integer>, Relation>();',
    ...
    def kod_get_class(self) -> List[str]:
        lines: List[str] = [
            ...
            'private final Map<Map.Entry<String, Integer>, Relation> relations;',
