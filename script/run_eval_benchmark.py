...
for i in os.listdir(args.path):
    name = re.match("eval-(.*?).pickle", i)
    if not name: continue
    basename = name.group(1)
    cmdline = ['python3', 'src/mypyvy.py', 'fol-benchmark-eval', f'--expt-flags={flags}', '--query', os.path.join(args.path, i), f'examples/fol/{basename}.pyv']
    print(" ".join(cmdline))
